...
- 5.如何處理缺失數據？（如果缺失的數據不可得，採用何種手段收集？）
  ...
  三大前提需先行釐清：
  1. 缺失數據的定義與缺失概況
  2. 缺失數據類別（例如：隨機缺失、完全隨機缺失、非隨機缺失）
  3. 缺失數據後續綜合應用，影響使用的處理方式
...
- 6.描述對漏斗模型的理解
- ```
  將使用者行為步驟拆解並逐步轉化為量化的模型，
  主要希望從漏斗各階層、各環節中達成以下幾個目的：
  ...
  舉例：
  1. 觸發某種觸及條件，且將商品放置於購物車達7天，系統便自動寄送優惠折扣碼連結至信箱
  2. 在特定頁面，將預設欄位設計為勾選狀態，並推薦用戶我們匹配相對應的有興趣商品
  3. 簡化步驟，讓使用者的輸入或前往下一步的阻力降低，更流暢有效率的使用流程
  
  ```
- 7.如何作用戶圖像
  ```
  我自己會採用設計思考的心法切割成五步驟執行：
  
  1. EMPATHY   
  訪談使用者，大量收集相關使用者信息，從真實存在的實際數據出發，不帶成見及預設立場的方法廣泛且仔細地收集資訊
  
  2. DEFINE    
  將廣且雜的數據精煉出最具代表性的特徵標識，
  將收集到的相關資料->數據清洗->梳理數據間的邏輯->脈絡釐清->彙整歸納
  
  3. IDEATE    
  設計處理資料標籤化的建模，不斷圍繞這些標籤去思考形象化且合情合理的用戶圖像
  
  4. PROTOTYPE 將這些標籤產出最陽春的初版圖像
  檢查是否符合以下七點
